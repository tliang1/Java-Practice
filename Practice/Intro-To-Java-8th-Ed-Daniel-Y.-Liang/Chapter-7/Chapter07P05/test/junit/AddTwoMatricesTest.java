...
public class AddTwoMatricesTest
{
	...
	public void testIsMatrix()
	{
		...
		{
			for (int matrix = 0; matrix < listsOfMatricesPairs[pair].length; matrix++)
			{
				assertEquals(validMatrices[currentMatrix], 
						AddTwoMatrices.isMatrix(listsOfMatricesPairs[pair][matrix]));
				currentMatrix++;
			}
		}
	}
	...
	public void testAddMatrix()
	{
		for (int pair = 0; pair < listsOfMatricesPairs.length; pair++)
		{
			double[][] sum = AddTwoMatrices.addMatrix(listsOfMatricesPairs[pair][0], listsOfMatricesPairs[pair][1]);
			
			for (int row = 0; row < sumsOfTwoMatrices[pair].length; row++)
			{
				assertArrayEquals(sumsOfTwoMatrices[pair][row], sum[row], 0.1);
			}
		}
	}
}